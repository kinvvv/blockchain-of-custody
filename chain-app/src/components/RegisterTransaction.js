...
const ToggleButton = styled.button.attrs({type: 'button'})`
  ...
  font-size: 1rem;
  border: none;
  background-color: ${props => props.isActive ? '#505F98' : 'transparent'};
  color: ${props => props.isActive ? '#FBFBFD' : '#505F98'};
  cursor: pointer;
  transition: all 0.3s ease;
  outline: none;

  &:hover {
    background-color: ${props => props.isActive ? '#505F98' : '#E8EAF2'};
  }
`;
...
const RegisterTransaction = () => {
  ...
  useEffect(() => {
    ...
    if (location.pathname === '/register-transaction') {
      setFormData(prevData => ({...prevData, status: '압수'}));
    }
  }, [navigate, location]);
  ...
  const handleStatusChange = (newType) => {
    setFormData(prevData => ({...prevData, imageType: newType}));
  };
        ...
        <Form onSubmit={handleSubmit}>
          ...
          <FormGroup>
            ...
            <ToggleWrapper>
              <ToggleContainer>
                  <ToggleButton 
                    isActive={formData.imageType === '선별 이미지'}
                    onClick={() => handleStatusChange('선별 이미지')}
                  >
                    선별 이미지
                  </ToggleButton>
                  <ToggleButton 
                    isActive={formData.imageType === '전부 이미지'}
                    onClick={() => handleStatusChange('전부 이미지')}
                  >
                    전부 이미지
                  </ToggleButton>
                </ToggleContainer>
